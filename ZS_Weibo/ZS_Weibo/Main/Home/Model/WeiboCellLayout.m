...
//        _weiboImageViewFrame = CGRectZero;
    ...
    if (_weibo.retweeted_status) {
        ...
        rect = [_weibo.retweeted_status.text boundingRectWithSize:CGSizeMake(kScreenWidth-4*SpaceWidth, 10000) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil];
    ...
    //---------转发微博背景图片-----------
        _reWeiboBgImageViewFrame = CGRectMake(SpaceWidth, _reWeiboTextFrame.origin.y-SpaceWidth, kScreenWidth-2*SpaceWidth, 2*SpaceWidth + _reWeiboTextFrame.size.height);
